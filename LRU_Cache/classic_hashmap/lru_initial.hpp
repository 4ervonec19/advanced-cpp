...
{
public:
    LRU_Cache(size_t N, size_t Bytes) : max_size(N), max_bytes(Bytes), current_bytes_size(0) {}
        ...
        if (it != cache_unordered_map.end())
            {
                ...
                current_bytes_size -= get_note_bytes(it->second->first, it->second->second);
    ...
    {  
        ...
        std::cout << "Current size: " << size() << ", Bytes used: " << size_bytes() << std::endl;
    }

private:
    size_t get_note_bytes(const keyT &input_key, const valueT &input_value)
    {
        return input_key.size() * sizeof(input_key[0]) + input_value.size() * sizeof(input_value[0]);
    }
    ...
    std::unordered_map<keyT, typename std::list<std::pair<keyT, valueT>>::iterator> cache_unordered_map;
};
